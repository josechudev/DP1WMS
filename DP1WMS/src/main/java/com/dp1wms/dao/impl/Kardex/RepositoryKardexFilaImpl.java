...
public class RepositoryKardexFilaImpl implements RepositoryKardexFila {
    ...
    @Override
    public List<KardexFila> selectAllKardexFila(String fecInicio,String fecFin,String producto, String tipoMovimiento) {
        String sql;
        if (!tipoMovimiento.equals("%todos%")){
             sql = "select \n" +
                    ...
                    "left join tipomovimiento as tm on tm.idtipomovimiento = m.idtipomovimiento\n" +
                    "left join producto as p on p.idproducto = dm.idproducto\n" +
                    "where m.fechamovimiento>= ?::date and m.fechamovimiento<=?::date and (lower(p.nombreproducto) like ?) and " +
                    "(lower(tm.descripcion) like ?)\n " +
                    "order by idmovimiento\n";

            return jdbcTemplate.query(sql, new Object[]{fecInicio,fecFin,producto,tipoMovimiento},new KardexFilaRowMapper());
            ...
            sql = "select \n" +
                    ...
                    "left join tipomovimiento as tm on tm.idtipomovimiento = m.idtipomovimiento\n" +
                    "left join producto as p on p.idproducto = dm.idproducto\n" +
                    "where m.fechamovimiento>= ?::date and m.fechamovimiento<=?::date and (lower(p.nombreproducto) like ?) \n"+
                    "order by idmovimiento\n";
            return jdbcTemplate.query(sql, new Object[]{fecInicio,fecFin,producto},new KardexFilaRowMapper());
        }
    }
}
